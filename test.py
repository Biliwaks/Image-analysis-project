...
def equation(list_path):
    ...
    for k in range(len(list_path)): #Separate "1" and "-"
        if list_path[k]==1 and k%2!=0 : list_path[k]=10
    ...
    if len(list_path)>0 and list_path[nb_elements-1] == 13 : #If we have equal
        #Calculer
...
images = main_segmentation(frame_init, plot=True)
image, image_idx = get_divide_sign(images,plot = True)
...
while(cap.isOpened()):
    ...
    cx, cy = object_center(arrow_frame)
    robot_posx.append(int(cy))
    robot_posy.append(int(cx))
